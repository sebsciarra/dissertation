...
Data for each simulation experiment were generated using R [@rstudio]. The code used to run the simulations and create the data set can be found in Appendix \ref{simulation-code}. To generate the data, the *multilevel logistic function* shown below in Equation \@ref(eq:logFunction-generation) was used:
...
Each data set generated by the multilevel logistic function (Equation \ref{eq:logFunction-generation}) was analyzed using a modified latent growth curve model known as a structure latent growth curve model [@preacher2015].
...
\end{align}
```
\noindent where $\sigma^2_{effect}$ represents the variance accounted by an effect and $MSE$ is the mean squared error. Importantly, $\sigma^2_{effect}$ values were corrected values obtained by using the following formula in Equation \ref{eq:var-effect} for a two-way factorial design with fixed variables [@howell2009]:
...
#### Nature of Change That Leads to Highest Model Performance {#nature-change-equal-exp1}
...
generate_day_likert_facet_plot(analytical_data = exp_1_analytical, target_col = 'measurement_spacing', target_value = 'Equal spacing',
                                ...
                                x_axis_var = 'midpoint', exp_num = 'exp11_', beta_lower = -30, beta_upper = 25, beta_ticks = 5)
```
...
#### Nature of Change That Leads to Highest Model Performance {#nature-change-time-inc-exp1}
...
#### Nature of Change That Leads to Highest Model Performance {#nature-change-time-dec-exp1}
...
-   fixed-effect triquarter-halfway delta parameter ($\upgamma_{fixed}$; Figure \ref{fig:exp1_plot_time_dec}B): five measurements with a nature-of-change value of 80.
...
set_panel_size(p = midpoint_280_plot, height = unit(x = 28, units = 'cm'),
                 width = unit(x = 40, units = 'cm'),
                 file =  'Figures/midpoint_280_plot.pdf')
